...
GitHub Pages 是网站的永久主站和 canonical。Netlify 部署同一份静态构建产物，提供完整备用镜像。只有在实际网络测试证明更快时，才通过可选变量让 Netlify 优先承载构建后的 WOFF2 字体和书籍封面。PDF、Tally、Z-Library、JavaScript、CSS 和其他装饰资源不经过 Netlify。

## 地址约定

- 主站：`https://fulinte1966.github.io/CulturalSimmer/`
- 镜像：`https://culturalsimmer-mirror.netlify.app/CulturalSimmer/`
- 可选资源源变量：`PUBLIC_ASSET_BASE_URL=https://culturalsimmer-mirror.netlify.app`

`PUBLIC_ASSET_BASE_URL` 必须是没有路径、查询参数和片段的 HTTPS origin。当前保持未设置，使本地开发和生产构建均使用同源资源；Netlify 完整镜像不受影响。
...
| Optional environment variable | `PUBLIC_ASSET_BASE_URL` | 仅在实测确认加速后设为 `https://culturalsimmer-mirror.netlify.app` |
...
启用可选资源源时，字体声明按 Netlify、GitHub Pages 的顺序包含两个 WOFF2 地址，封面图片记录同源回退地址，镜像加载失败时只重试一次。未启用时，字体与封面直接使用 GitHub Pages 同源地址。Netlify 镜像发送 `X-Robots-Tag: noindex`，所有页面的 canonical 均指向 GitHub Pages。
