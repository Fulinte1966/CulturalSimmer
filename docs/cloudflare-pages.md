...
Cloudflare 发布失败时，主站保留上一版，GitHub Pages 仍可独立发布；通知工作流只在 Cloudflare 主站成功上线后即时触发。需要临时停用 Cloudflare 时，禁用 `deploy-cloudflare` job 或删除 `cloudflare-pages` Environment，GitHub Pages 不依赖 Cloudflare 凭据。
