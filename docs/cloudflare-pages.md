...
候选发布与删除预览继续通过 `cloudflare-pages` Environment 使用同一最小权限部署身份，避免复制 Cloudflare secret。另建不持有部署凭据的 `ebook-production` Environment 并设置 required reviewers；正式发布或公开删除 job 必须先通过该批准关卡。两个预览分支域名 `ebook-preview.fulinte.pages.dev` 与 `removal-preview.fulinte.pages.dev` 必须由 Cloudflare Access 应用以默认拒绝策略保护。GitHub Environment 只负责凭据或批准，不能代替 Cloudflare Access。
