...
候选生成若修正了标题、副标题或其他会进入历史更新文字的书目字段，预览与晋级
都会在完整验证前重建网站更新归档。归档检查仍保持只读门禁；不得通过跳过
`updates:archive:check` 发布过期归档。
...
editions:
  ...
  - edition: 2
    editionDate: "2026-07"
    releaseTag: F-1-1_v2
    manifest: src/data/manifests/F-1-1_v2.json
---
```
...
```txt
src/data/manifests/F-1-1_v1.json
src/data/manifests/F-1-1_v2.json
```
