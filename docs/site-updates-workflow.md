...
正式 PDF 入库会显式触发带 `notify_updates=true` 的部署；Cloudflare Pages 与
GitHub Pages 都成功后，部署工作流立即 dispatch QQ 与 ntfy。普通代码部署、
定时天气刷新和未带参数的手动部署不会广播。通知仓库的定时检查继续作为独立
兜底；两个通道使用独立状态文件、共享固定节点操作锁和稳定事件 ID，即时触发、
重复触发与后续定时检查都不会重复发送。人工公告仍由定时检查发现，除非另有
明确的发布自动化。
