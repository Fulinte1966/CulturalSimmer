...
正式 PDF 入库和网站部署不会即时广播。QQ 与 ntfy 在北京时间每日 06:00 同时进入固定节点队列，由共享操作锁串行执行；08:00 再进行一次幂等补跑。两个通道使用独立状态文件和稳定事件 ID，已发送事件不会重复。人工公告同样等待下一次每日广播；只有明确的通道测试或运维处置才手动运行 live workflow。
