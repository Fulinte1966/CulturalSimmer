...
`cloudflare-dist/CulturalSimmer/downloads/` 是部署时生成的临时目录，只包含从 GitHub Releases 下载并通过 manifest 摘要校验的最新版 PDF。该目录和 PDF 不属于网站源码，不得复制到 `public/` 或提交到 Git。详情页源 HTML 仍以 GitHub Release 为下载地址，Cloudflare 包装阶段只改写带专用标记的链接。
