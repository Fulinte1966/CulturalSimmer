...
Historical PDFs without complete XMP metadata must not pass strict ingestion. A
local `--assets-only` option may continue generating cover, spine, outline, and
reading data for legacy PDFs, but GitHub ingestion must always use strict
metadata mode.
