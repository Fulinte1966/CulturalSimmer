...
分类代码必须唯一，子级代码必须以前级代码开头。分类层级只由 YAML 中的 `children` 关系决定，不由代码长度或分隔点推断。YAML 中的代码只保存大写字母和数字，不写索书号分隔点；加载时会从字母后的第一位开始，每三个数字自动插入 `.`，例如 `I2104` 显示为 `I210.4`、`K9263` 显示为 `K926.3`，但后者仍可与 `K926` 同为三级分类。索引页、分类页、书号校验和 PDF 上传流程均读取这一个文件；修改后应运行 `npm run validate` 和相关测试。
